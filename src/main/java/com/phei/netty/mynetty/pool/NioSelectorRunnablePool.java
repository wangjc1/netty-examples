package com.phei.netty.mynetty.pool;

import com.phei.netty.mynetty.NioServerBoss;
import com.phei.netty.mynetty.NioServerWorker;

import java.util.concurrent.Executor;
import java.util.concurrent.atomic.AtomicInteger;

/**
 * selector线程管理者:用于初始化服务线程和消费线程
 * @author -琴兽-
 *
 * 注：可以看出这个线程有个问题，就是一开始就初始化好了所有线程，不管有没有任务要执行
 */
public class NioSelectorRunnablePool {
	...
	private void initBoss(Executor boss, int count) {
		...
		for (int i = 0; i < bosses.length; i++) {
			//初始化服务端线程，Selector.open()打开selector，同时启动线程
